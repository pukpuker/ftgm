...
class GetPPMod(loader.Module):
    ...
    async def potocmd(self, message):
        ...
        if len(args)>1:
            try: id = int(args[1])-1
            except: return await message.edit("not integer")
            ...
            if g:
                ...
                os.system(f'ffmpeg -y -i "{pfp}" \
                    -f lavfi -i anullsrc=channel_layout=stereo:\
                        sample_rate=44100 \
                            -c:v copy -shortest "{name}"')
                f.append(name)
            else:
                pfp = await self.client.download_media(
                    tr, f"{entity.id}_{tr.date}")
                f.append(pfp)
